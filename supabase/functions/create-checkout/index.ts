...
function validateInput(data: CheckoutRequest): {
  ...
  if (
    !["one_time", "monthly", "quarterly", "yearly"].includes(data.donationType)
  ) {
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // IMPORTANT: Do NOT send donor thank-you here to avoid duplicates.
    // For membership fees, send a pending payment email with checkout link.
    if (purpose === "membership_fee" && email) {
      try {
        await supabaseClient.functions.invoke("notify-emails", {
          body: {
            ...
            payload: {
              ...
              amount, // dollars string, matches template expectation
              checkoutUrl: session.url,
            },
          },
        });
      } catch (emailError) {
        console.error("Pending membership email failed:", emailError);
      }
    }
